...
void spi_transfer(spi_hander_t spi, void *data, unsigned int size)
{
 ...
 || (defined(BSP_USING_SPI3) && defined(BSP_SPI3_TX_USING_DMA) && defined(BSP_SPI3_RX_USING_DMA))
    if ((pspi->tx_dma_ch >= 0) && (pspi->rx_dma_ch >= 0))
    {
        // 设置传输地址和数据大小
        LL_DMA_ConfigAddresses(DMA1, pspi->rx_dma_ch, (uint32_t)&pspi->spi->DR, (uint32_t)data, LL_DMA_DIRECTION_PERIPH_TO_MEMORY);
        LL_DMA_SetDataLength(DMA1, pspi->rx_dma_ch, size);
        LL_DMA_ConfigAddresses(DMA1, pspi->tx_dma_ch, (uint32_t)data, (uint32_t)&pspi->spi->DR, LL_DMA_DIRECTION_MEMORY_TO_PERIPH);
        LL_DMA_SetDataLength(DMA1, pspi->tx_dma_ch, size);
        //使能DMA，使能DMA传输
        LL_DMA_EnableChannel(DMA1, pspi->rx_dma_ch);
        LL_DMA_EnableChannel(DMA1, pspi->tx_dma_ch);
        LL_SPI_EnableDMAReq_RX(pspi->spi);
        LL_SPI_EnableDMAReq_TX(pspi->spi);

        //等待传输完成
        os_msg_recv(&pspi->transfer_msg, 0, 1000);

        //关闭DMA传输和DMA
        LL_SPI_DisableDMAReq_TX(pspi->spi);
        LL_SPI_DisableDMAReq_RX(pspi->spi);
        LL_DMA_DisableChannel(DMA1, pspi->tx_dma_ch);
        LL_DMA_DisableChannel(DMA1, pspi->rx_dma_ch);
    } else
#endif
    {
        // 不使用dma
        unsigned int sync = 0;
        if (LL_SPI_GetDataWidth(pspi->spi) == LL_SPI_DATAWIDTH_16BIT)
        {
            uint16_t *txbuf = data;
            uint16_t *rxbuf = data;
            while(size)
            {
                if ((pspi->spi->SR & SPI_SR_TXE) && (sync == 0))
                {
                    pspi->spi->DR = *txbuf;
                    txbuf++;
                    sync = 1;
                ...
                if (pspi->spi->SR & SPI_SR_RXNE)
                {
                    *rxbuf = (uint16_t)pspi->spi->DR;
                    rxbuf++;
                    size--;
                    sync = 0;
                }
        ...
        else
        {
            ...
            while(size)
            {
                if ((pspi->spi->SR & SPI_SR_TXE) && (sync == 0))
                {
                    *((__IO uint8_t*)&pspi->spi->DR) = *txbuf;
                    txbuf++;
                    sync = 1;
                }
                ...
                if (pspi->spi->SR & SPI_SR_RXNE)
                {
                    *rxbuf = *((__IO uint8_t*)&pspi->spi->DR);
                    rxbuf++;
                    size--;
                    sync = 0;
                }
...
void spi_send(spi_hander_t spi, const void *data, unsigned int size)
{
 ...
 || (defined(BSP_USING_SPI3) && defined(BSP_SPI3_TX_USING_DMA)) 
    ...
    // 清空接收缓存
    while(pspi->spi->SR & SPI_SR_RXNE)
    {
        __IO unsigned int tmp = pspi->spi->DR;
        (void)tmp;
    }
    LL_SPI_ClearFlag_OVR(pspi->spi);
