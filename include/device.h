...
static inline int device_write(struct device *dev, const void *data, unsigned leng)
{
    return dev->ops->write(dev, data, leng);
}

static inline int device_read(struct device *dev, void *data, unsigned leng)
{
    return dev->ops->read(dev, data, leng);
}
